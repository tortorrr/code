...
int main() {
  ...
  printf("\nDHCP Discover: ");
  recvfrom(server_socket, &yiaddr, sizeof(yiaddr), 0,
           (struct sockaddr *)&server_address, &client_address_size);
  recvfrom(server_socket, &transID, sizeof(transID), 0,
           (struct sockaddr *)&server_address, &client_address_size);
  printf(" yiaddr: %s, transId: %d\n", yiaddr, transID);

  fscanf(f, "%[^\n]", yiaddr);
  printf("\nDHCP offer:");
  sendto(server_socket, &yiaddr, sizeof(yiaddr), 0,
         (struct sockaddr *)&server_address, sizeof(server_address));
  sendto(server_socket, &transID, sizeof(transID), 0,
         (struct sockaddr *)&server_address, sizeof(server_address));
  sendto(server_socket, &lifetime, sizeof(lifetime), 0,
         (struct sockaddr *)&server_address, sizeof(server_address));
  ...
  recvfrom(server_socket, &transID, sizeof(transID), 0,
           (struct sockaddr *)&server_address, &client_address_size);
  recvfrom(server_socket, &lifetime, sizeof(lifetime), 0,
           (struct sockaddr *)&server_address, &client_address_size);
  ...
  while (1) {
    ...
    if (elapsed >= (lifetime / 2)) {
      recvfrom(server_socket, &choice, sizeof(choice), 0,
               (struct sockaddr *)&server_address, &client_address_size);
      if (choice != 1) {
        break;
      } else {
